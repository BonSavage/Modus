...
(defun eval-none(pattern)
  (mvb (_ succ) (eval-pattern (expr-predicate pattern))
       (unless (or _ succ)
...
(defun sweepedp(pattern)
  (find pattern *sweeped* :test (lambda (pred swpd) (partialp (predicate-body pred) (predicate-body swpd)))))
...
(defun infsub(pattern)
  (mvb (subs succ) (rule-inference pattern)
       (if succ
	   (mapcar (lambda (sub) (predicate-sublis sub pattern)) subs)
	   nil)))
